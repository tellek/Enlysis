...
////// Specular:
                ...
                fixed node_3219 = 0.2;
                float3 specularColor = ((_SpecularMap_var.rgb-(_CloudMap_var.rgb*_CloudMapSpecularity))*(_Smoothness+node_3219));
...
/////// Diffuse:
                ...
                float3 diffuseColor = (_CloudMap_var.rgb+_MainTex_var.rgb);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                fixed4 _CloudMap_var = tex2D(_CloudMap,TRANSFORM_TEX(node_3503, _CloudMap));
                fixed4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (_CloudMap_var.rgb+_MainTex_var.rgb);
                fixed4 _SpecularMap_var = tex2D(_SpecularMap,TRANSFORM_TEX(i.uv0, _SpecularMap));
                fixed node_3219 = 0.2;
                float3 specColor = ((_SpecularMap_var.rgb-(_CloudMap_var.rgb*_CloudMapSpecularity))*(_Smoothness+node_3219));
