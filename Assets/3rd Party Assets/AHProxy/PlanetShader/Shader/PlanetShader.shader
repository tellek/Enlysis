...
Shader "AHProxy/PlanetShader" {
    Properties {
        ...
        _SpecularMapIntensity ("SpecularMapIntensity", Float ) = 1
...
////// Specular:
                ...
                float3 diffuseColor = (node_2392+(_MainTex_var.rgb*_TextureColor.rgb)); // Need this for specular when using metallic
...
////// Emissive:
                float3 emissive = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),(8.0-_AtmosphereDensity))*_AtmosphereColor.rgb)*saturate((dot(i.normalDir,lightDirection)+_LightStretch)));
/// Final Color:
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                o.Emission = ((pow(1.0-max(0,dot(normalDirection, viewDirection)),(8.0-_AtmosphereDensity))*_AtmosphereColor.rgb)*saturate((dot(i.normalDir,lightDirection)+_LightStretch)));
                ...
                float3 node_2392 = (_CloudMap_var.rgb*_CloudColor.rgb);
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float3 diffColor = (node_2392+(_MainTex_var.rgb*_TextureColor.rgb));
                float4 _SpecularMap_var = tex2D(_SpecularMap,TRANSFORM_TEX(i.uv0, _SpecularMap));
